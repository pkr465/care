...
bool prplmesh_cli::print_radio(std::string device_path)
{
    ...
    if (!ht_radio) {
        LOG(ERROR) << "Unable to access object at path " << radio_ht_path;
        return false;
    }
    int radio_index = 1;
    ...
    amxc_htable_iterate(radio_it, ht_radio)
    {
        ...
        if (!ht_bss) {
            LOG(ERROR) << "Unable to access object at path " << bss_ht_path;
            radio_index++;
            continue;
        }
        int vap_index = 0;
        ...
        amxc_htable_iterate(bss_it, ht_bss)
        {
            ...
            if (!ht_sta) {
                LOG(ERROR) << "Unable to access object at path " << sta_ht_path;
                vap_index++;
                continue;
            }
            int sta_index = 1;
...
bool prplmesh_cli::prpl_conn_map()
{
    ...
    if (!devices) {
        LOG(ERROR) << "Unable to access object at path " << conn_map.device_ht_path;
        return false;
    }
