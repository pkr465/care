...
// ISSUE #1: Use async-signal-safe type for signal communication.
static volatile sig_atomic_t s_signal = 0;

static void init_signals()
{
    auto signal_handler = [](int signum) { s_signal = static_cast<sig_atomic_t>(signum); };
...
{
    const auto signum = static_cast<int>(s_signal);

    switch (signum) {
    case SIGTERM:
    case SIGINT:
        LOG(INFO) << "Caught signal '" << strsignal(signum) << "' Exiting...";
        g_running = false;
        break;
    case SIGUSR1: {
        if (!s_pLogger) {
        ...
        LOG(INFO) << "Caught signal '" << strsignal(signum) << "' Rolling log file...";
...
#endif // ENABLE_NBAPI
    default:
        LOG(WARNING) << "Unhandled Signal: '" << strsignal(signum) << "' Ignoring...";
        break;
    }
