                                           ...
                                           amxc_var_t *const retval, void *priv)
{
    /*
        ...
        Then, we get current time and subtract retrieved timestamp for getting time passed
    ...
    */
    ...
    if (last_steer_timestamp <= current_time) {
        last_steer_time = current_time - last_steer_timestamp;
    }
                                           ...
                                           amxc_var_t *const retval, void *priv)
{
    ...
    // Initialization of ambiorix triggers read action and it leads un-initalized db object
    if (!g_database) {
                                             ...
                                             amxc_var_t *const retval, void *priv)
{
    /*
        ...
        Then we retrieve CurrentTime and subtract CreationTime for getting time passed
    ...
    */
    ...
    if (creation_time <= current_time) {
        last_change = current_time - creation_time;
    }
                                  ...
                                  amxc_var_t *ret)
{
    ...
    if (network_enable) {
        amxd_object_for_each(instance, it, access_point)
        {
            ...
            if (!security_inst) {
                LOG(ERROR) << "Security object is missing for AccessPoint: " << bss_info.ssid;
                continue;
            }
                           ...
                           amxc_var_t *ret)
{
    ...
    if (pool_size < 0 || pool_size > int(channel_pool.size())) {
        LOG(ERROR) << "channels_num out of range: " << pool_size;
    ...
    for (auto channel_str : channels_vec) {
        ...
        if (i >= pool_size || i >= int(channel_pool.size())) {
            LOG(ERROR) << "Too many channels provided";
            return amxd_status_invalid_value;
        }
        ...
        channel_pool[i++] = static_cast<uint8_t>(channel_num);
    }
...
/**
 * @brief Initiates setting spatial reuse parameters and bss color with the Creation TLV at the current
 * Radio and the given parameters
 *
 * Example of usage:
 ...
 *
 * If some input parameter(s) are not provided, then the corresponding existing parameter in
 * SpatialReuse applies.
 *
 */
                                       ...
                                       amxc_var_t *args, amxc_var_t *ret)
{
    ...
    // If some parameter(s) are not provided then the corresponding existing parameter in SpatialReuse
    // applies.
    uint32_t bss_color;
 ...
 * '{"vbssid": "aa:bb:cc:dd:ee:ff", "client_mac": "aa:bb:cc:dd:ee:ff", "ssid": "prplMeshNetwork",
 * "pass": "prplmeshpass"}'
 *
 */
 ...
 * '{"client_mac" : "aa:bb:cc:dd:ee:ff", "dest_ruid" : "aa:bb:cc:dd:ee:ff", "ssid":
 * "prplMeshNetwork", "pass": "prplmeshpass"}'
 *
 */
 ...
 *
 * event_group_enable_changed is invoked when value of parameter
 * Device.WiFi.DataElements.Network.Group.X.Enable is changed
 *
 */
 ...
 *
 * event_group_enable_changed is invoked when value of parameter
 * Device.WiFi.DataElements.Network.Enable is changed
 *
 */
