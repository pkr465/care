...
{
    optind = 1; // reset getopt() state for a fresh parse
    int opt;
    char *ap   = nullptr;
    char *ssid = nullptr;
    ...
    return ap && ssid && prpl_cli.set_ssid(ap, ssid);
}
...
bool set_security(int argc, char *argv[])
{
    ...
    return ap && mode && prpl_cli.set_security(ap, mode, passphrase ? passphrase : "");
}
